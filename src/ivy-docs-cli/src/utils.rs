...
pub fn get_relative_folder_without_order(input_folder: &Path, input_file: &Path) -> String {
    ...
    let normalized_file = input_file
        .parent()
        .unwrap_or(Path::new(""))
        .to_str()
        .unwrap_or("")
    ...
    let rel = if normalized_file.starts_with(&normalized_input) {
        ...
        if trim < normalized_file.len() && normalized_file[trim..].starts_with('/') {
            trim += 1;
        }
...
pub fn rename_class(code: &str, old_class: &str, new_class: &str) -> String {
    ...
    ctor_re
        .replace(&code, format!("public {new_class}("))
        .to_string()
}

lazy_static! {
    static ref GIT_INFO: Mutex<Option<(String, String, String)>> = Mutex::new(None);
}
...
pub fn get_path_for_link(source: &str, link: &str) -> String {
    ...
    let source_dir = if let Some(idx) = source_normalized.rfind('/') {
        source_normalized[..idx].to_string()
    } else {
        "".to_string()
    };
    ...
    if source_dir.is_empty() {
        ...
        format!("{}/{}", source_dir, link_str).replace("\\", "/")
    }
}
...
pub fn title_case_to_friendly_url(mut input: String) -> String {
    ...
    if input.to_lowercase().ends_with("app") {
...
pub fn get_app_id_from_type_name(type_name: &str) -> String {
    ...
    if let Some(pos) = ns.iter().position(|&x| x == "Apps") {
        ns = ns[pos + 1..].to_vec();
    }
    let friendly: Vec<String> = ns
        .iter()
        .map(|&s| title_case_to_friendly_url(s.to_string()))
        .collect();
    friendly.join("/")
}
