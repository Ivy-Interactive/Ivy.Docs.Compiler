...
pub fn convert_async(
    ...
    if let Some(ref imports) = app_meta.imports {
        for import in imports {
            code_builder.push_str(&format!("using {};\n", import));
        }
    }
    ...
    if let Some(ref icon) = app_meta.icon {
        code_builder.push_str(&format!(", icon:Icons.{}", icon));
    }
    if let Some(ref title) = app_meta.title {
        code_builder.push_str(&format!(", title:{}", utils::format_literal(title)));
    }
    if app_meta.group_expanded {
        code_builder.push_str(", groupExpanded:true");
    }
    if app_meta.hidden {
        code_builder.push_str(", isVisible:false");
    }
    ...
    code_builder.push_str(&format!(
        "public class {}(bool onlyBody = false) : {}\n{{\n",
    ...
    if has_content {
        ...
        for (id, text, level) in headings {
            headings_code.push_str(&format!(
                "new ArticleHeading({}, {}, {}), ",
                utils::format_literal(&id),
                utils::format_literal(&text),
                level
            ));
        }
        headings_code.push('}');
        ...
        code_builder
            .push_str("            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()\n");
...
fn handle_blocks(
    ...
    let write_section = |section: &mut String,
                         code_builder: &mut String,
                         referenced_apps: &mut HashSet<String>,
                         _remove_bottom_margin: bool| {
        ...
        if !trimmed.is_empty() {
            ...
            let prepend = if is_nested_content {
                ", new Markdown("
            } else {
                "| new Markdown("
            };
    ...
    let details_matches: Vec<(usize, usize, String)> = DETAILS_BLOCK
        .find_iter(markdown_content)
        ...
        for child in &root.children {
            ...
            match child {
                Node::Html(html) => {
                    ...
                    if html_str.starts_with("<details>")
                        || html_str.to_lowercase().starts_with("</")
                    {
                        continue;
                    }

                    if html_str.starts_with("<Callout")
                        || html_str.starts_with("<Embed")
                ...
                Node::Heading(heading) => {
                    ...
                    if let Some(ref mut heads) = headings {
                        heads.push((
                            generate_heading_id(&text),
                            text.trim().to_string(),
                            heading.depth as i32,
                        ));
                    }
                }
        ...
        for line in raw.lines() {
            sb.push_str(&format!(
                ...
                if line.is_empty() {
                    ...
                    format!("{}{}", "    ".repeat(tabs + 1), line.trim_end())
                }
            ));
        }
...
fn handle_details_direct(
    ...
    let summary_match = SUMMARY_START
        .find(html_content)
        .expect("Details block missing <Summary>");
    let summary_start = summary_match.start() + summary_match.len();
    let summary_end = html_content[summary_start..]
        .find("</Summary>")
        .expect("Details missing closing </Summary>")
        + summary_start;
    let summary = html_content[summary_start..summary_end].trim();

    let body_match = BODY_START
        .find(html_content)
        .expect("Details block missing <Body>");
    let body_start = body_match.start() + body_match.len();
    let body_end = html_content
        .rfind("</Body>")
        .expect("Details missing closing </Body>");
    ...
    handle_blocks(
        &ast,
        body_content,
        &mut body_code_builder,
        view_builder,
        used_class_names,
        &mut body_referenced_apps,
        &body_link_converter,
        false,
        4,
        None,
    ...
    if !body_output.is_empty() {
        ...
        let lead = if base_indent > 3 { ", " } else { "| " };
        code_builder.push_str(&format!(
            "{}{}new Expandable(\"{}\", new Markdown(\"No content\"))\n",
            "    ".repeat(base_indent),
            lead,
            summary
        ));
    ...
    match layout {
        "demo-tabs" => {
            ...
            append_multiline(
                ...
                &format!(",{}))", map_language_to_enum(language)),
            );
            code_builder.push_str(&format!(
        ...
        _ => {
            append_demo_content(code_builder, base_indent, &insert_code);
        }
    }
}
...
fn handle_widget_docs_block(
    code_builder: &mut String,
    xml: &roxmltree::Node,
    headings: Option<&mut Vec<(String, String, i32)>>,
) {
    let type_name = xml
        .attribute("Type")
        .expect("WidgetDocs block must have Type");
    let ext_types = xml.attribute("ExtensionTypes");
    let src_url = xml.attribute("SourceUrl");

    let e = if let Some(x) = ext_types {
        utils::format_literal(x)
    } else {
        "null".to_string()
    };
    let s = if let Some(x) = src_url {
        utils::format_literal(x)
    } else {
        "null".to_string()
    };
        ...
        for line in code_content.lines() {
            if line.starts_with('>') {
                code_builder.push_str(&format!(
                    "{}.AddCommand({})\n",
                    "    ".repeat(base_indent + 1),
                    utils::format_literal(line.trim_start_matches('>').trim())
                ));
            } else {
                code_builder.push_str(&format!(
                    "{}.AddOutput({})\n",
                    "    ".repeat(base_indent + 1),
                    utils::format_literal(line.trim())
                ));
            }
        }
        code_builder.push_str(&format!("{}\n", "    ".repeat(base_indent + 1)));
